...
//      明文裡（同一支天線就抓得到），種類也是。
//      **範圍限定 —— 但這個限定比想像中弱，兩點務必一起讀**：
//        (i)  能偽造 STATE 的人同樣能偽造 CMD 直接驅動繼電器，所以本次改動新增的
//             不是「控制面」而是「說謊面」。**但攻擊者其實連製造失敗都不必**：
//             本系統的靜止狀態是繼電器 OFF ＝ 門開（見 ho_slave1 的 30 秒失聯保護），
//             所以**單純干擾就足以造成關門失敗**，偽造只要負責「掩蓋」。
//        (ii) 偽造 CMD 的效果是**暫態**的（HO_CMD_ON 撐不過 slave 的 30 秒失聯保護，
//             HO_CMD_PULSE 更短），而**一封偽造的 STATE 是持續的** —— 它會一直留在
//             master 的 slaves[] 與 MQTT 上，並且**抑制「去現場確認」這個系統唯一的
//             真防線**。就危害而言，說謊面比控制面更久、更難察覺。
//      謊的方向正是誤綠（「門關好了」而門是開的），是本系統最不能接受的失敗型態。
