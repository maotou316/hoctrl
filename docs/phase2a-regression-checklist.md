...
```
> **Phase 2b Task 4 更新**：訂閱動作已從連線函式抽到訂閱排程器
> （`controlSubscribeScheduler()`，每輪 `loop()` 訂一條），所以「已連線」與
> 「已訂閱」變成**兩行**（原本是同一行的 `[MQTT] 已連線 X，訂閱 Y`），
> 而且「已訂閱」會比「已連線」晚一點點出現（正常網路下是毫秒級）。
> 若名冊上有 slave，後面還會逐台印 `[代理] 已訂閱 hoban/<slaveId>/control`，
> 每台一行 —— 這是正常輸出，不是失敗。
>
> **失敗判定（Task 4 review M4 新增）**：只要看到
> `⚠ [MQTT] 訂閱失敗 …` 或 `⚠ [代理] 訂閱失敗 …` 就是 **FAIL**。
> `已訂閱` 這一行只在 `mqttClient.subscribe()` 真的回傳成功時才印，
> 所以「沒看到 `已訂閱`、卻看到 `⚠ … 訂閱失敗`」代表該 topic 沒訂上，
> 對應的設備收不到 App 指令。**不要因為連線那一行成功就判 PASS。**
