...
### 8a. `alloff` 對一台**收不到廣播**的 slave 必須誠實回報（Task 5 review C1 的守衛）

> **這一項的存在理由**：Task 5 第一版有一條假綠燈 —— 用 master 自己送的
> `HO_PKT_STATE_REQ` 製造「新回報」，再配上 slave 的靜止預設值 `relay == 0`，
> 把一台**從未收到 OFF** 的 slave 判成「已確認」，而且從此不再補送。
> **原本的清單走完一輪抓不到它**（第 8 項只測全部正常的情況）。這一項就是為了
> 讓那種缺陷在實測時一定現形。

- 操作：
  1. 至少配對 2 台 slave，全部在線。
  2. 讓其中一台**收不到 master 的封包**但仍在名冊上 —— 最容易重現的做法是
     把該台移到訊號邊界處或用金屬罐體屏蔽；**直接斷電也可以**（斷電那台會
     同時收不到廣播與單播，正是要測的情境）。
  3. master 輸入 `alloff`。
- 預期：
  - 序列埠**必須**出現 `⚠ [群組] 1 台連「送達」都沒有（單播沒拿到 MAC 層 ACK）`
    與 `⚠ [群組]   未送達：hoban-<那一台>`。
  - 收工那行的 `已送達 X／Y` 的 X **必須小於** Y。
  - 補送必須真的發生：要看到 `[群組] 第 2 趟補送：…`（至少一次）。
- **什麼情況算 FAIL（這才是本項的重點）**：
  - 序列埠印出「全部送達」「已逐台確認」「全部回報預期狀態」之類的字樣；
  - 或那台完全沒有出現在未送達清單裡；
  - 或一次補送都沒發生。
- **附帶檢查（MQTT）**：訂閱 `hoban/<masterId>/status`，該則訊息必須含
  `"group":{…,"noack":1,…,"exec":"unprovable"}`，且那一台在 `slaves[]` 裡的
  `"grp"` 必須是 `0`。`"exec"` 不是 `"unprovable"` 即為 FAIL。
- [ ] 通過
