...
### 8e. **N1 的完整實機驗證**：peer 被刪、之後又補回來（`droppeer` ＋ 重新配對）

> **這是唯一一項在 N1 修正被還原時會 FAIL 的測試。** 8a／8a-2 碰不到觸發條件，
> 8d 碰得到觸發條件卻造不出症狀（假 slave 永遠不是 peer，之後的單播同樣送不出去、
> 不會產生任何可被誤記的 ACK）。
>
> N1 的症狀需要同一個 MAC 在 **T1「送不出去」**、在 **T2「送得出去且拿到 ACK」**。
> `droppeer <n>` 製造 T1（刪 peer、保留名冊條目 → `findSlave()` 成功所以不會被標
> `gone`，但 `esp_now_send()` 回 `ESP_ERR_ESPNOW_NOT_FOUND`）；
> 配對模式下短按該台按鈕製造 T2（`ho_slave1` 的 `requestPairing()` **沒有**
> 「已配對就不送」守衛，master 的 `HO_PKT_PAIR_REQ` 分支**無條件** `registerPeer()`
> 後單播 `PAIR_ACK`，peer 就此復原）。
...
- 操作：
  ...
  3. `alloff`。預期：
     ...
     - slave#1 每一趟都是成對的兩行：`[控制] 送指令 2 給 hoban-<#1>`
       **緊接** `[ESP-NOW] esp_now_send 失敗: <碼>`（＝ N1 觸發前提標記）。
     - 收工 `已送達 1／2`，未送達清單只含 slave#1。
     - **關鍵**：slave#1 是快照的最後一個索引，所以收工當下**閂正是為它開著**
       —— 這正是 N1 的前置狀態。
  4. **收工之後**：master 短按 BOOT 進配對模式（LED 慢閃）→ 短按 slave#1 的按鈕。
     預期 master 印 `[配對] 接受 hoban-<#1>，目前共 2 台`。
  5. 觀察之後 **60 秒**的 `hoban/<masterId>/status`，期間**不要再下任何群組指令**。
- **判定（可完全區分修正在／不在）**：
  - **有 N1 修正（現行版本）**：slave#1 的 `"grp"` **恆為 `0`**、`"group"` 的
    `"noack"` **恆為 `1`**。因為 `groupSendUnicast()` 送不出去當場關閂、
    `groupFinishJob()` 開頭再關一次、且收工後 `phase == IDLE` 讓
    `groupNoteUnicastAck()` 直接 return —— 三道都擋。
  - **N1 修正被還原**：那則 `PAIR_ACK` 的 MAC 層 ACK（或緊接著 `pollNextSlave()`
    的 `STATE_REQ` ACK）會命中**還開著**的閂 → slave#1 的 `"grp"` 由 `0` 翻成 `1`、
    `"noack"` 由 1 減成 0。**假綠燈現形。**
- **FAIL 條件**：第 5 步觀察期間 slave#1 的 `"grp"` 出現 `1`，或 `"noack"` 減成 0。
- **附帶效果**：本項同時也是 `HO_PKT_PAIR_ACK` 那條同 MAC 窗口
  （第 5 輪 review N-b 已加關閂）的第一個實機驗證。
