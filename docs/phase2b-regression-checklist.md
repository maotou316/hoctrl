...
```json
"group": {"cmd":2,"age_s":<秒>,"busy":0,"n":2,"ack":2,"noack":0,"gone":0,"exec":"unprovable"}
```
且 `slaves[]` 每筆有 `"grp":1`。

**【失敗判定】**
- **序列埠或 MQTT 任何一處宣稱「已確認執行」「已關門」「全部回報預期狀態」**
  → FAIL。韌體被要求只能宣稱「已送達」。
- 少印上面那兩行「不能證明已執行」的說明（**無論送達與否都必須印**）。
- `"exec"` 不是 `"unprovable"`。
...
> `:1816-1829` 的 `appendGroupResult()`（`exec` 固定 `"unprovable"`）。
