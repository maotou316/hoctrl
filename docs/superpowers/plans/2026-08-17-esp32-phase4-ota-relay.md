...
> **⚠ Task 5 複審第 2 輪的更正（N4）：上表的 `"target"` 那一列是 30，實際上界是 31，
> 合計是 119 不是 118。**
> 30 是拿範例 `hoban-aabbccddeeff`（**18** 字元）算的，而真正的上界是
> `otaTargetId[20]` 容得下的 **19** 字元：`"target":` 9 ＋ 引號含內容 21 ＋ 逗號 1 ＝ **31**。
> **這張表隱含了一條沒寫出來的第四條前提**：「id ≤ 18 字元」。現行程式碼的實際前提是
> 「`otaTargetId` 的容量是 20 bytes」，已寫進 `ho_master1.ino` 的常數註釋，
> 並由 `tools/check_doc_claims.py` 方向 19 釘住那個宣告。
> **128 這個上界在兩種算法下都成立**（方向保守），照抄不會做錯東西，但數字要以
> 程式碼註釋為準，不要照抄這張表的 30／118。
