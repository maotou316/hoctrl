...
## 系統本質（2026-08-16 由使用者澄清，最高優先需求）

**這是一套「多開門的捕捉系統」，核心需求是「一次要全部關」。**

這句話決定了群組控制的實作方式，也推翻了 Phase 1 的既有寫法：

- **群組指令必須用 ESP-NOW 廣播，不可逐台單播。** Phase 1 的 `sendCmdToAll()` 是逐台單播、每台間隔 20ms，20 台就是 **400ms 落差**——最後一道門比第一道晚將近半秒。對捕捉系統而言，那半秒就是動物的逃脫窗口
- 廣播是一個封包送到同一個無線電時隙，所有 slave 的落差是**微秒級**
- 代價是廣播沒有 ACK。所以完整流程是：**廣播（真正同時）→ 事後逐台查狀態（誰沒關上）→ 對未確認的補送單播**
- 補送是「補救」不是「主要路徑」，不可以退化成逐台送

**這也意味著「全部關」的可靠性優先於「全部開」。** 關門失敗 = 動物逃脫或未被捕捉；開門失敗 = 下次再開即可。設計取捨時以「關」為準。
