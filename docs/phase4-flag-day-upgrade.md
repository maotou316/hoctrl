...
| **先 master 後 slave** | master 一升級，**所有尚未升級的 slave 同時**失去心跳 | 30 秒後**全部**一起 `setRelayPins(false)` —— 這是「一次要全部關」的系統最不能接受的同步失敗。**但暴露面有明確邊界，見下方** |

> **「先 master」到底會傷到哪些籠子 —— 現場的人用這一段判斷自己有沒有暴露。**
>
> 30 秒失聯保護只在 `relayState` 為 true 時才動作（`startChannelScan()` 開頭的
> `if (relayState)`）。而**本系統的關門路徑走的是 `HO_CMD_PULSE`**
>（App 的「全部關門」送 `ALL:ON` → `sendCmdToAll(HO_CMD_PULSE, 2000)`），
> 點動 2 秒後 slave 自己就把繼電器關掉了 —— **靜止狀態是 OFF**。
>
> 所以「先 master」的實際傷害面**只涵蓋當下正在用 `HO_CMD_ON` 持續通電維持的籠子**
>（只有序列埠的 `allon`／`on <n>`、或 MQTT 代理層走到 `HO_CMD_ON` 的路徑會產生這種狀態）。
>
> **判斷方法**：升級前在 master 下 `list`，看有沒有任何一台的 `relay` 是 `1`。
> 全部是 `0` ⇒ 這一批沒有暴露在這條傷害面上（**窗口本身仍然存在** ——
> 那 30 秒內任何關門指令都不會被執行，第 1 節的前提照樣要做）。
...
// ⚠ 時間條件**必須**寫成無號數相減再轉有號比較，不可以寫成 lastCmdAt >= startedAt。
// 兩者只在 millis() 約 49.7 天迴繞時行為不同，而那一次的差別是：
// 直接比大小會讓迴繞後的「新回報」看起來比「舊指令」更早 → 條件不成立 → 誤紅；
// 更糟的是同一個寫法用在別處（例如點動結束判定）會變成誤綠。全檔一律用這個形式。
&& (long)(slaves[s].lastCmdAt - groupJob.startedAt) >= 0
```

（`groupExecutedIdx()` 裡是等價的早退寫法
...
**但這個限定比它看起來弱一階，兩點必須一起讀**：

1. **攻擊者連「製造失敗」都不必，只要「掩蓋」。**
   本系統的靜止狀態是**繼電器 OFF ＝ 門開**（`startChannelScan()` 的 30 秒失聯保護
   就是把繼電器關掉）。所以**單純的射頻干擾本身就足以造成關門失敗** ——
   把廣播與單播全部蓋掉，門就是開的。偽造 STATE 的角色只是讓畫面顯示成綠的。
2. **偽造 CMD 是暫態的，偽造 STATE 是持續的。**
   一封偽造的 `HO_CMD_ON` 撐不過 slave 的 30 秒失聯保護（`HO_CMD_PULSE` 更短），
   攻擊者要維持效果就得持續發送；而**一封偽造的 `HO_PKT_STATE` 會一直留在**
   master 的 `slaves[]` 與 MQTT 上，直到下一道指令為止。
   更關鍵的是它**抑制了「去現場確認」—— 這個系統唯一的真防線**。

也就是說：就危害的**持續時間**與**被察覺的機率**而言，說謊面比控制面更糟。
...
| 要求同時有 MAC 層 ACK（`grp==1`）才承認 `exe` | ❌ 否決 | 會毀掉本次歸因最有價值的情形（只收到廣播的那台是「送達紅、執行綠」）；**而且完全無效** —— MAC 層 ACK 是由收到單播的那台射頻**自動**產生的，攻擊者只要用被害 slave 的 MAC 冒名接收就能讓 ACK 出現，不需要那台真的執行任何動作 |
...
把 ota 誠實列進去後 `(3072-1-728-11)/112 = 2332/112 = 20`，**恰好等於上限而餘裕歸零** ——
Task 5 一放進去就撞牆。
