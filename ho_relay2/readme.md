...
**症狀**：接 USB 一切正常（燈號、WiFi、MQTT 遠端控制都好），拔掉 USB 只靠電池時
面板 LED 長亮、設備不上線，板載 MOS 也帶不動負載。

**根因（2026-09-23 實例）**：**電池線虛焊**，造成供電電壓不足。

**為什麼「長亮」就能斷定韌體沒在跑**：上面的 LED 狀態表裡**沒有任何常亮的狀態**——
BLE 配對是 200ms 快閃、WiFi 未連是 300ms 快閃或心跳閃、MQTT 未連是一長二短、
都連上則熄燈，唯一的長亮是長按重置確認那 0.7 秒（`relayOn()` 雖然會讓 LED 長亮，
但它沒有任何呼叫點，是死碼）。所以面板燈穩定長亮 ＝ 晶片沒在執行韌體、
GPIO 0 沒人驅動而停在高電位；同一時間 GPIO 4/7 當然也沒人驅動，負載不會動。

**為什麼接 USB 就正常**：USB 進來的是穩定、低內阻的 5V；電池經 VIN 進來還要再過
板上穩壓器，而虛焊的接點等於在路徑上串了一顆大電阻。空載時電表量起來完全正常，
一帶載（ESP32-C3 啟動 WiFi 瞬間電流 300mA 以上）電壓立刻塌、3.3V 維持不住，MCU 起不來。

### 診斷順序

1. **先量電池電壓夠不夠。** 2S 鋰電標稱 3.7V × 2 = **7.4V**，滿電 8.4V，放電下限 6.0V。
   量到 5V 或更低就不必再查別的了。
2. **空載正常不代表沒問題。** 虛焊、線徑不足、接觸電阻大都屬於「電壓夠、電流不夠」，
   必須**帶載量測**——只接電池、按一下板上 RESET，同時盯著板子端電壓會不會塌。
   按 RESET 仍起不來，就不是上電重置失效，而是供電帶不動。
3. 確認虛焊就重新焊接點，再只接電池測一次。

### 不要一開始就懷疑主板

判斷主板好壞最快的方法是接 USB：MCU、WiFi、MQTT、韌體邏輯與 GPIO 輸出若在 USB
供電下全部正常，主板就是好的。不必拆機也能驗到韌體那一層——訂閱
`hoban/<id>/status`，請現場的人從 App 按一次，看 `device.relay` 有沒有跳 1→0，
間隔應為 2 秒（`pulseRelay()` 的 `delay(2000)`）。板載 LED 會在同一段程式碼裡同步亮
2 秒，可用來確認 GPIO 輸出層有沒有動作。

**下任何指令前必須先確認設備身分**（序列埠開機 log 的 MAC、或 App 上的設備 ID），
不可拿 MQTT 掃出來的清單猜——掃到的每一台都是運作中的現場設備，
繼電器動作是不可復原的實體後果。詳見 `.claude/rules/mqtt-command-device-identity.md`。
...
起因是現場一台 1.8.x 設備在分享器停電後再也連不上，重連邏輯讀完沒有卡點，code review 反而在
電池／按鈕區塊抓到三個問題，第一個就能完整解釋症狀：

1. **運行中電池斷開會誤觸重置**（`isResetButtonPressed()`）。BMS 低壓斷電、接頭鬆脫、桌測 USB
   供電時拔電池，S 腳只剩模組下臂 7.5 kΩ 通到地，ADC 讀 ≈0 mV，和按鈕短路到地分不出來 →
   3 秒＋2 秒確認後 `clearWiFiConfig()` → 設備重啟進 BLE 配對模式，之後電源／分享器怎麼回來都
   不會上線。「分享器斷線」很常等於現場停電，這條路完全對得上。
   修法：讀值 <500 mV 時**開內部上拉再讀一次**——按鈕短路仍是 ≈0，7.5 kΩ 到地是 ≈470
   （就是開機指紋「模組沒電 ≈0/470」那個數字），腳浮空是 ≈1.9～2.2 V（100nF 5 ms 只充到六成）；只有 <250 mV
   （`BATTERY_SHORT_MV`）才算按鈕。上拉只在低讀值時開 5 ms，正常量電壓路徑不受影響。
   順帶把 rules 檔原本列在「擋不住」的「沒焊 100k 上拉時模組脫落浮空」也擋掉了。
   被拒絕的低讀值會限頻（60 秒一次）印
   `⚠ 電量檢測: GPIO 1 讀值 <500 mV 但開上拉後為 N mV，不是按鈕`，現場接序列埠看得出來。
2. **`checkStuckButtons()` 會把 `detectBatterySense()` 的保護蓋掉**。它用
   `resetButtonUsable = (count < total)` 無條件覆寫，而「模組接了但沒供電」那格（開上拉 <2000 mV）
   剛設下的停用就被清掉。0.8～2.0 V 是數位輸入灰色地帶，10 次取樣抖一次 HIGH 就翻回可用。
   改成只往停用方向改（`if (stuck) usable = false`）。
3. **開機第一筆電量永遠是空的**。setup 的「先量一次」`updateBatteryReading()` 必被 5 秒限頻擋掉
   （`lastBatteryReadTime` 初值 0、此時 `millis()` 才 1 秒多），第一筆 retained 上線 status 的
   `battery` 是 `{mv:0, percent:-1, valid:false}`。改成 `detectBatterySense()` 量到的那筆直接寫入。

**它擋不住什麼**：上拉確認靠的是內部上拉（約 45 kΩ，晶片間 30～60 kΩ 都有）與模組下臂
7.5 kΩ 的分壓比，落點 367～660 mV，門檻 250 有餘裕但不是無限：典型 45 k 上拉時下臂臨界 ≈3.7 kΩ、
最弱 60 k 時 ≈4.9 kΩ，下臂低於 5 kΩ 的模組就要重算；5 ms 的餘裕也是以 S 腳 100 nF 為前提，
焊成 1 µF 時 5 ms 後只有 ≈254 mV，貼著門檻。1.8.3 以前的現場設備仍會被這條路清掉設定，只能 OTA 升級。
另外，「電池斷開 → 誤觸重置」要成立，**MCU 在電池斷開時必須仍有電**（另一路供電或 USB）；
若現場那台就是這顆 2S 電池單獨供電，BMS 斷電時 ESP 一起斷電，跑不到 5 秒重置，這個修正就不是它的原因。

**實測狀態**：僅編譯通過（2026-09-19），尚未在實機驗證「拔電池不觸發重置」與「按鈕仍可重置」兩個方向，
也尚未確認現場那台的供電拓撲是否允許「電池斷開而 MCU 仍運行」。
