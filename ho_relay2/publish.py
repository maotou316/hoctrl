...
# 硬體變體定義（同款 ESP32-C3，不同出廠年份 GPIO 不同）
VARIANTS = [
...
def build_firmware(model, variant):
    """編譯單一硬體變體的韌體"""
    print_header(f"編譯韌體: {model} (GPIO {variant['relay_pin']})")
    ...
    build_path = f"build/{model}"

    # 創建 build 目錄
    os.makedirs(build_path, exist_ok=True)

    # 透過編譯旗標指定 GPIO（型號由 .ino 根據 RELAY_PIN 自動決定）
    extra_flag = f'-DRELAY_PIN={variant["relay_pin"]}'

    print_color(f"正在編譯 (使用自訂 Partition Scheme)...", Colors.YELLOW)
...
def main():
    ...
    print_color("║   (同時發布所有硬體變體)             ║", Colors.CYAN)
    print_color("╚════════════════════════════════════════╝\n", Colors.CYAN)
    ...
    for v in VARIANTS:
        print_color(f"變體: {v['model']} (GPIO {v['relay_pin']})", Colors.WHITE)
